...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if txCode == types.MintTx {
		...
		rtCmt := tx.RTcmt()
		err = zktx.VerifyMintProof(tx.ZKSN(), &rtCmt, tx.ZKCMT(), tx.ZKValue(), tx.ZKProof())
		if err != nil {
			return err
		}
	}
	...
	if txCode == types.TransferTx {
		...
		// If this is a receiver tx with both proofs, verify proof_B (type=1) as well
		if tx.ZKProof2() != nil && len(tx.ZKProof2()) > 0 {
			err = zktx.VerifyTransferProof(tx.ZKCMTS(), tx.ZKSNS(), tx.ZKCMT2(), &rtCmt, tx.ZKValue(), 1, tx.ZKProof2())
			if err != nil {
				return err
			}
		}
	}
	if txCode == types.CreateAccountTx {
		err = zktx.VerifyCreateAccountProof(tx.ZKCMT(), tx.ZKProof())
		if err != nil {
			return err
		}
	}
