...
var CreateAccountTxStr string = "CreateAccount Transaction"
...
//
func (tx *Transaction) ZKCMT2() *common.Hash {
	return tx.data.ZKCMT2
}

//
func (tx *Transaction) SetZKCMT2(hash *common.Hash) {
	tx.data.ZKCMT2 = hash
}
...
//
func (tx *Transaction) ZKProof2() []byte {
	return tx.data.ZKProof2
}

//
func (tx *Transaction) SetZKProof2(proof []byte) {
	tx.data.ZKProof2 = make([]byte, len(proof))
	copy(tx.data.ZKProof2[:], proof[:])
}
...
func (tx *Transaction) GetTxCodeStr() string {
	if tx.Code() == PublicTx {
		...
		return RedeemTxStr
	} else if tx.Code() == TransferTx {
		return TransferTxStr
	} else if tx.Code() == CreateAccountTx {
		return CreateAccountTxStr
	} else {
