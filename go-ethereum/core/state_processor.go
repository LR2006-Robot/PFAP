...
func ApplyTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config) (*types.Receipt, uint64, error) {
	...
	if tx.TxCode() == types.MintTx {
		...
		rtCmt := tx.RTcmt()
		if err = zktx.VerifyMintProof(tx.ZKSN(), &rtCmt, tx.ZKCMT(), tx.ZKValue(), tx.ZKProof()); err != nil {
		...
		rtCmt := tx.RTcmt()
		if err = zktx.VerifyRedeemProof(tx.ZKSN(), &rtCmt, tx.ZKCMT(), tx.ZKValue(), tx.ZKProof()); err != nil {
		...
		if err = zktx.VerifyTransferProof(tx.ZKCMTS(), tx.ZKSN(), tx.ZKCMT(), &rtCmt, tx.ZKValue(), 0, tx.ZKProof()); err != nil {
			fmt.Println("invalid zk transfer proof_A: ", err)
			return nil, 0, err
		}
		if tx.ZKProof2() != nil && len(tx.ZKProof2()) > 0 {
			if err = zktx.VerifyTransferProof(tx.ZKCMTS(), tx.ZKSNS(), tx.ZKCMT2(), &rtCmt, tx.ZKValue(), 1, tx.ZKProof2()); err != nil {
				fmt.Println("invalid zk transfer proof_B: ", err)
				return nil, 0, err
			}
		}
		statedb.CreateAccount(common.BytesToAddress(tx.ZKSN().Bytes()))
