...
func (s *PublicTransactionPoolAPI) SendMintTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	if zktx.SequenceNumber.SN == nil || zktx.SequenceNumberAfter.SN == nil {
		fmt.Println("SequenceNumber.SN or SequenceNumberAfter.SN nil")
		return common.Hash{}, nil
	}
	...
	if exist == false && *(zktx.SequenceNumber.SN) != *(zktx.InitializeSN().SN) {
		fmt.Printf("WARNING [mint]: zktx.SequenceNumber.SN not on chain but not initial, SN=%x (SequenceNumberAfter unchanged)\n", zktx.SequenceNumber.SN[:])
	}
	...
	if SN == nil || SN.SN == nil {
		fmt.Println("SequenceNumberAfter or its SN is nil")
		return common.Hash{}, nil
	}
	tx.SetZKSN(SN.SN) //SN
...
func (s *PublicTransactionPoolAPI) SendSendTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) { //tbd
	...
	if exist == false && *(zktx.SequenceNumber.SN) != *(zktx.InitializeSN().SN) {
		fmt.Printf("WARNING [send]: zktx.SequenceNumber.SN not on chain but not initial, SN=%x (SequenceNumberAfter unchanged)\n", zktx.SequenceNumber.SN[:])
	}
	...
	// Look up the account's current balance
	SN := zktx.SequenceNumberAfter
	tx.SetZKSN(SN.SN) //SN
	...
	fmt.Println("***** Create send transaction Cost Time (ms): ", time.Since(txCreateStart).Nanoseconds()/1000000, " Tx Size (bytes): ", signed.Size())
...
func (s *PublicTransactionPoolAPI) SendDepositTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	if exist == false && *(zktx.SequenceNumber.SN) != *(zktx.InitializeSN().SN) {
		fmt.Printf("WARNING [deposit]: zktx.SequenceNumber.SN not on chain but not initial, SN=%x (SequenceNumberAfter unchanged)\n", zktx.SequenceNumber.SN[:])
	}
...
loop:
	...
	fmt.Println("***** Create deposit transaction Cost Time (ms): ", time.Since(txCreateStart).Nanoseconds()/1000000, " Tx Size (bytes): ", signedTx.Size())
...
func (s *PublicTransactionPoolAPI) SendRedeemTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	if exist == false && *(zktx.SequenceNumber.SN) != *(zktx.InitializeSN().SN) {
		fmt.Printf("WARNING [redeem]: zktx.SequenceNumber.SN not on chain but not initial, SN=%x (SequenceNumberAfter unchanged)\n", zktx.SequenceNumber.SN[:])
	}
...
func (s *PublicTransactionPoolAPI) SendCreateAccountTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	tx := args.toTransaction()
	tx.SetTxCode(types.CreateAccountTx)
	tx.SetPrice(big.NewInt(0))
	tx.SetValue(big.NewInt(0))
	tx.SetZKAddress(&zktx.ZKTxAddress)

	skA := zktx.NewRandomHash()
	snA := zktx.ComputePRF(skA.Bytes(), common.Hash{}.Bytes())
	rA := zktx.NewRandomHash()
	cmtA := zktx.GenCMT(0, snA.Bytes(), rA.Bytes())

	tx.SetZKCMT(cmtA)

	txCreateStart := time.Now()
	zkProof := zktx.GenCreateAccountProof(skA, rA, snA, cmtA)

	if string(zkProof[0:10]) == "0000000000" {
		return common.Hash{}, errors.New("can't generate createAccount proof")
	}
	tx.SetZKProof(zkProof)
...
func (s *PublicTransactionPoolAPI) SendTransferTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	// New flow: receiver gets payer data directly via args (RS, Seq, CmtANew, SnAOld, ProofA)
	if args.RS != nil && args.CmtANew != nil && args.SnAOld != nil && args.ProofA != nil {
		return s.sendTransferReceiverWithArgs(ctx, args)
	}

	// Old flow: lookup payer's on-chain tx via TxHash
	if args.TxHash != (common.Hash{}) {
		txPayer := s.GetTransactionByHash2(ctx, args.TxHash)
		if txPayer == nil || txPayer.ZKProof() == nil || len(txPayer.ZKProof()) == 0 {
			return common.Hash{}, errors.New("payer tx not found or invalid")
		}
		return s.sendTransferReceiver(ctx, args, txPayer)
	}

	return s.sendTransferPayer(ctx, args)
}
...
if zktx.SequenceNumber == nil || zktx.SequenceNumberAfter == nil {
	...
	exist = state.Exist(common.BytesToAddress(zktx.SequenceNumber.SN.Bytes()))
	if exist == false && *(zktx.SequenceNumber.SN) != *(zktx.InitializeSN().SN) {
		fmt.Printf("WARNING [transferPayer]: zktx.SequenceNumber.SN not on chain but not initial, SN=%x (SequenceNumberAfter unchanged)\n", zktx.SequenceNumber.SN[:])
	}
	...
	if SN.Value < valueS || valueS <= 0 {
		return common.Hash{}, errors.New("no sufficient balance or non-positive transfer amount")
	}

	newSN := zktx.ComputePRF(SK.Bytes(), SN.SN.Bytes())
	newValue := SN.Value - valueS
	newRandom := zktx.NewRandomHash()
	newCMT := zktx.GenCMT(newValue, newSN.Bytes(), newRandom.Bytes())
	...
	tx.SetZKCMTS(cmtS)
	// Store r_s in AUX for receiver to extract later
	...
	txCreateStart := time.Now()
	zkProof := zktx.GenTransferProof(SN.Value, SN.Random, newSN, newRandom, SN.CMT, SN.SN, newCMT, newValue, SK, valueS, rS, CMTSForMerkle, RTcmtBytes, 0)

	if string(zkProof[0:10]) == "0000000000" {
		return common.Hash{}, errors.New("can't generate transfer proof")
	}
	tx.SetZKProof(zkProof)

	// Refresh nonce after long proof generation
	if freshNonce, err := s.b.GetPoolNonce(ctx, args.From); err == nil {
		*(*uint64)(args.Nonce) = freshNonce
		tx.SetNonce(freshNonce)
	}
...
if zktx.SequenceNumber == nil || zktx.SequenceNumberAfter == nil {
	...
	exist = state.Exist(common.BytesToAddress(zktx.SequenceNumber.SN.Bytes()))
	if exist == false && *(zktx.SequenceNumber.SN) != *(zktx.InitializeSN().SN) {
		fmt.Printf("WARNING [transferReceiver]: zktx.SequenceNumber.SN not on chain but not initial, SN=%x (SequenceNumberAfter unchanged)\n", zktx.SequenceNumber.SN[:])
	}

	// Extract payer A's data from the on-chain tx
	cmtS := txPayer.ZKCMTS()
	snAold := txPayer.ZKSN()
	cmtANew := txPayer.ZKCMT()
	...
	if err := zktx.VerifyTransferProof(cmtS, snAold, cmtANew, &rtCmt, valueS, 0, proofA); err != nil {
	...
	newValueB := SNb.Value + valueS
	newRandomB := zktx.NewRandomHash()
	...
	rS := zktx.NewRandomHash()
	// For receiver, we need r_s from the payer. In a real flow, B gets this offline.
	// For the test, since the payer's AUX stores (value_s, r_s, sn_A_old), decode it.
	aux := txPayer.AUX()
	if len(aux) > 0 {
		val, rs, _ := zktx.DecAUXTransfer(aux)
		if val == valueS {
			rS = rs
		}
	}
...
func (s *PublicTransactionPoolAPI) sendTransferReceiverWithArgs(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	if zktx.SequenceNumber.SN == nil || zktx.SequenceNumberAfter.SN == nil {
		return common.Hash{}, errors.New("SequenceNumber.SN or SequenceNumberAfter.SN nil")
	}
	state, _, err := s.b.StateAndHeaderByNumber(ctx, rpc.LatestBlockNumber)
	if state == nil || err != nil {
		return common.Hash{}, err
	...
	exist := state.Exist(common.BytesToAddress(zktx.SequenceNumberAfter.SN.Bytes()))
	if exist == true && *(zktx.SequenceNumberAfter.SN) != *(zktx.InitializeSN().SN) {
		return common.Hash{}, errors.New("sn is lost")
	}
	exist = state.Exist(common.BytesToAddress(zktx.SequenceNumber.SN.Bytes()))
	if exist == false && *(zktx.SequenceNumber.SN) != *(zktx.InitializeSN().SN) {
		fmt.Printf("WARNING: zktx.SequenceNumber.SN not on chain but not initial, SN=%x (SequenceNumberAfter unchanged)\n", zktx.SequenceNumber.SN[:])
	}
	...
	// Compute cmt_S from value_s and r_s
	...
	// Set both payer's and receiver's data in the tx
	tx.SetZKCMTS(cmtS)
	tx.SetZKCMT(args.CmtANew)
	tx.SetZKCMT2(newCMTB)
	tx.SetZKSN(args.SnAOld)
	tx.SetZKSNS(SNb.SN)
	tx.SetZKProof([]byte(*args.ProofA))
	tx.SetZKProof2(nil)
	tx.SetZKValue(valueS)
	tx.SetRTcmt(rtCmt)
	...
	// Refresh nonce after long proof generation (42+ seconds)
	if freshNonce, err := s.b.GetPoolNonce(ctx, args.From); err == nil {
		*(*uint64)(args.Nonce) = freshNonce
		tx.SetNonce(freshNonce)
	}
...
func (s *PublicTransactionPoolAPI) GetPayerNextState(ctx context.Context, rs hexutil.Bytes, value hexutil.Uint64) (map[string]interface{}, error) {
	...
	if zktx.SequenceNumber.SN == nil || zktx.SequenceNumberAfter.SN == nil {
		return nil, errors.New("SequenceNumber.SN or SequenceNumberAfter.SN nil")
	}
	state, _, err := s.b.StateAndHeaderByNumber(ctx, rpc.LatestBlockNumber)
	if state == nil || err != nil {
		return nil, err
	...
	exist := state.Exist(common.BytesToAddress(zktx.SequenceNumberAfter.SN.Bytes()))
	if exist == true && *(zktx.SequenceNumberAfter.SN) != *(zktx.InitializeSN().SN) {
		return nil, errors.New("sn is lost")
	}
	exist = state.Exist(common.BytesToAddress(zktx.SequenceNumber.SN.Bytes()))
	if exist == false && *(zktx.SequenceNumber.SN) != *(zktx.InitializeSN().SN) {
		fmt.Printf("WARNING [GetPayerNextState]: zktx.SequenceNumber.SN not on chain but not initial, SN=%x (SequenceNumberAfter unchanged)\n", zktx.SequenceNumber.SN[:])
	}

	SN := zktx.SequenceNumberAfter
	valueS := uint64(value)
	if SN.Value < valueS || valueS <= 0 {
		return nil, errors.New("no sufficient balance or non-positive transfer amount")
	}
	...
	if err := zktx.VerifyTransferProof(cmtS, SN.SN, newCMT, &RTcmt, valueS, 0, proofA); err != nil {
		fmt.Println("***** GetPayerNextState: proof verification FAILED: ", err)
