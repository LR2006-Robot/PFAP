...
**State commitment tree (new):** existence of `cmt_old` is now proven against a single **global depth-256 sparse Poseidon Merkle tree (the state Merkle tree)**, shared by Mint / Redeem / Transfer. Inside the circuit the leaf is `path = Poseidon(cmt_old)` and all tree nodes use Poseidon; the commitment `cmt` itself is still computed with SHA-256. This replaces the previous per-transaction, `seq`-based SHA-256 Merkle tree, so callers **no longer pass a `seq` block list** — the witness (path + 256 siblings + root `rt_cmt`) is generated by the C++ side from the global tree. The tree is an in-memory singleton (in `libzk_smt.so`) that every node rebuilds deterministically by replaying confirmed commitments during block processing.
...
### Performance (new Poseidon-SMT scheme)

Measured on the tested environment; setup is one-time/offline. `pk size` / `vk size` are the actual serialized key sizes (≈ half the hex `.txt` file on disk). `pk load time` is the per-process cost to deserialize the proving key into memory on the **first** proof of each type (CPU-bound parsing, cached for subsequent same-type txs in the same process).

| Circuit       | QAP degree | Setup (gen) time | pk size  | vk size  | pk load time |
| ------------- | ---------- | ---------------- | -------- | -------- | ------------ |
| CreateAccount | 131,072    | ~30 s            | 16.90 MB | 238.88 B | ~30 s        |
| Mint          | 294,912    | ~70 s            | 42.19 MB | 318.62 B | ~75 s        |
| Redeem        | 294,912    | ~70 s            | 42.20 MB | 318.62 B | ~74 s        |
| Transfer      | 327,680    | ~78 s            | 46.77 MB | 358.50 B | ~82 s        |

The Go-side "Create Transaction time" ≈ `pk load time + proof-gen time + overhead`; the proof functions print proof-gen time separately, so subtract the `pk load time` above to isolate it.
...
//    Merkle tree, so NO block `seq` is needed anymore.
var valueS = "0x10"
var rs     = "0x01"
