...
export CFLAGS="-Wno-deprecated-declarations -Wno-format-overflow -Wno-unused-variable -Wno-unused-parameter -Wno-unused-but-set-variable -Wno-return-type"
export CXXFLAGS="${CFLAGS} -Wno-deprecated-copy -Wno-range-loop-construct -Wno-catch-value"
export CGO_CFLAGS="-Wno-format-overflow"
CMAKE_EXTRA_FLAGS="-Wno-deprecated -Wno-dev"
...
check_deps() {
    ...
    if [ ${#missing[@]} -gt 0 ]; then
        error "Missing build dependencies: ${missing[*]}\n  sudo apt-get install build-essential cmake git libgmp3-dev libboost-all-dev libssl-dev libproc2-dev pkg-config"
    fi
}
...
build_geth() {
    ...
    cd "$geth_build_dir"
    info "Running: GO111MODULE=off go install -tags generic -v ./cmd/geth  (from $geth_build_dir)"
    GO111MODULE=off go install -tags generic -v ./cmd/geth
