...
alt_bn128_pp::init_public_params();
    ...
    if (!pk_loaded) {
        cached_pk = deserializeProvingKeyFromFile("/usr/local/prfKey/sendpk.txt");
        pk_loaded = true;
    }
    ...
    proof = generate_send_proof<alt_bn128_pp>(keypair.pk, note_old, notes, note_new, cmtA, cmtS , cmtAnew, sk, pk_sender);
...
bool verifySendproof(char *data, char *cmtA_old_string, char *sn_old_string, char *cmtS_string ,char *cmtA_new_string)
{
    ...
    if (!vk_loaded) {
        cached_vk = deserializevkFromFile("/usr/local/prfKey/sendvk.txt");
        vk_loaded = true;
    }
    r1cs_gg_ppzksnark_keypair<alt_bn128_pp> keypair;
