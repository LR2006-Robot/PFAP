...
int main(){
    ...
    createAccount_gadget<FieldT> ca(pb);
    ca.generate_r1cs_constraints();
    const r1cs_constraint_system<FieldT> constraint_system = pb.get_constraint_system();
    r1cs_gg_ppzksnark_keypair<alt_bn128_pp> keypair = r1cs_gg_ppzksnark_generator<alt_bn128_pp>(constraint_system);
    serializeProvingKeyToFile(keypair.pk,"createaccountpk.txt");
    vkToFile(keypair.vk,"createaccountvk.txt");
}
