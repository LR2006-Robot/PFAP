...
std::string HexStringFromLibsnarkBigint(libff::bigint<libff::alt_bn128_r_limbs> _x)
{
    ...
    std::stringstream ss;
    ss << std::setfill('0');
    for (unsigned i = 0; i < 32; i++) ss << std::hex << std::setw(2) << (int)x[i];
...
{
    return outputPointG1AffineAsHex(proof.g_A) + outputPointG2AffineAsHex(proof.g_B) + outputPointG1AffineAsHex(proof.g_C);
}
    ...
    uint256 sk_data, uint256 r_A_data, uint256 sn_A_data, uint256 cmtA_data)
{
    ...
    if (!pb.is_satisfied()) {
        cout << "can not generate createAccount proof" << endl;
...
char *genCreateAccountproof(char *sk_string, char *r_A_string, char *sn_A_string, char *cmtA_string)
{
    uint256 sk = uint256S(sk_string);
    uint256 r_A = uint256S(r_A_string);
    uint256 sn_A = uint256S(sn_A_string);
...
alt_bn128_pp::init_public_params();
    ...
    if (!pk_loaded) {
        cached_pk = deserializeProvingKeyFromFile("/usr/local/prfKey/createaccountpk.txt");
        pk_loaded = true;
    }
...
bool verifyCreateAccountproof(char *data, char *cmtA_string)
{
    ...
    if (!vk_loaded) {
        cached_vk = deserializevkFromFile("/usr/local/prfKey/createaccountvk.txt");
        vk_loaded = true;
    }
    r1cs_gg_ppzksnark_keypair<alt_bn128_pp> keypair;
    ...
    for (int i = 0; i < 64; i++)
    {
        A_x[i] = uint8_t(data[i + 0]); A_y[i] = uint8_t(data[i + 64]);
        B_x_1[i] = uint8_t(data[i + 128]); B_x_0[i] = uint8_t(data[i + 192]);
        B_y_1[i] = uint8_t(data[i + 256]); B_y_0[i] = uint8_t(data[i + 320]);
        C_x[i] = uint8_t(data[i + 384]); C_y[i] = uint8_t(data[i + 448]);
    }
    ...
    bool result = verify_createAccount_proof(keypair.vk, proof, cmtA);
    ...
    printf("\n\n verify createAccount proof Use Time:%fs\n\n", verify_timeuse);
    fflush(stdout);
    if (!result) cout << "Verifying createAccount proof unsuccessfully!!!" << endl;
    // else cout << "Verifying createAccount proof successfully!!!" << endl;
    return result;
}
