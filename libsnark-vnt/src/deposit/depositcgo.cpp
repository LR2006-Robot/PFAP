                      ...
                      char *sk_string)
{
    ...
    MerklePath path;
    ZCIncrementalWitness wit = ZCIncrementalMerkleTree().witness();
    try {
        ZCIncrementalMerkleTree tree;
        assert(tree.root() == ZCIncrementalMerkleTree::empty_root());
        ...
        if (!find_cmtS) {
            printf("deposit: cmtS not found in commitments\n");
            fflush(stdout);
        ...
        printf("deposit: merkle tree error: %s\n", e.what());
        fflush(stdout);
        char *p = new char[1153];
        memset(p, '0', 1152);
        *(p + 1152) = '\0';
...
alt_bn128_pp::init_public_params();
    ...
    if (!pk_loaded) {
        cached_pk = deserializeProvingKeyFromFile("/usr/local/prfKey/depositpk.txt");
        pk_loaded = true;
    }
...
bool verifyDepositproof(char *data, char *RT, char *pk, char *cmtb_old, char *snold, char *cmtb, char *sns)
{
    ...
    if (!vk_loaded) {
        cached_vk = deserializevkFromFile("/usr/local/prfKey/depositvk.txt");
        vk_loaded = true;
    }
    r1cs_gg_ppzksnark_keypair<alt_bn128_pp> keypair;
    ...
    printf("\n\n verify deposit proof Use Time:%fs\n\n", verify_timeuse);
    fflush(stdout);
